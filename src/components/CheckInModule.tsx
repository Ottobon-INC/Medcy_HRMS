...
export default function CheckInModule({
 ...
 const punchTypeBadge = (punchType?: PunchType) => {
  if (!punchType || punchType === 'in_office') {
   return (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-[#f3edfb] text-[#7e3acb] rounded-full text-[9px] font-bold border border-teal-100">
  ...
  <div id="check-in-module-container"className="space-y-6">
   ...
   <div id="status-panel"className="bg-white rounded-[32px] p-8 shadow-sm border border-slate-100 flex flex-col md:flex-row items-center justify-between gap-6 transition-all duration-300">
    ...
    <div id="live-timer-badge"className="bg-slate-50 border border-slate-100 rounded-2xl px-6 py-4 flex items-center gap-4 shadow-sm">
     <div className={`p-3 rounded-xl ${isOnBreak ? 'bg-amber-100 text-amber-600' : 'bg-[#f3edfb] text-[#8a42db]'}`}>
     ...
     <div>
      ...
      {isCheckedIn && (
       isOnBreak ? (
        ...
        <span className="text-[9px] font-bold text-[#8a42db] uppercase tracking-widest animate-pulse block mt-0.5">
         ● {t.runningLive}
        </span>
       )
      )}
    ...
    <button
     ...
     className={`group flex flex-col items-center justify-center w-48 h-48 md:w-52 md:h-52 rounded-full border-[12px] shadow-md transition-all duration-300 active:scale-95 disabled:opacity-60 ${
      isOnBreak
       ...
       : 'border-teal-50 bg-[#8a42db] hover:bg-[#7e3acb] shadow-teal-100 text-white cursor-pointer'
     }`}
    >
     {isProcessing ? (
   ...
   <div id="logs-timeline"className="bg-white rounded-[24px] p-6 shadow-sm border border-slate-100">
    <div className="flex items-center gap-2 mb-6">
     <ListCollapse className="w-4 h-4 text-[#8a42db]"/>
           ...
           <td className="py-3 px-4">
            <div className="flex items-center gap-2">
             {log.checkOutTime ? (
              ...
              <span className="inline-flex items-center gap-1.5 font-mono text-[9px] font-bold bg-purple-100 text-[#6a2baf] px-2.5 py-0.5 rounded-full animate-pulse uppercase tracking-wider">
               {t.runningLive}
              </span>
             )}
     ...
     <div className="bg-white rounded-2xl overflow-hidden max-w-md w-full shadow-md">
      <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
       <h3 className="font-bold text-slate-800 flex items-center gap-2">
        <Camera className="w-5 h-5 text-[#8a42db]"/>
      ...
      <div className="relative bg-black aspect-video flex items-center justify-center">
       ...
       <div className="absolute inset-0 border-4 border-[#8a42db]/30 m-4 rounded-xl pointer-events-none"></div>
      </div>
      ...
      <div className="p-6 flex flex-col items-center">
       ...
       <button
        ...
        className="bg-[#8a42db] hover:bg-[#7e3acb] text-white font-bold py-3 px-8 rounded-full shadow-sm hover:shadow-teal-500/25 transition-all flex items-center gap-2 w-full justify-center active:scale-95"
       >
      ...
      <div className="p-6 space-y-4">
       ...
       <div className="pt-2 flex flex-col gap-3">
        <button
         ...
         disabled={isPinning}
         className="w-full py-3 bg-[#8a42db] hover:bg-[#7e3acb] text-white font-bold rounded-xl transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
        >
     ...
     <div className="bg-white rounded-2xl overflow-hidden max-w-md w-full shadow-md">
      ...
      <div className="p-6 flex flex-col items-center">
       <button
        ...
        className="bg-[#8a42db] hover:bg-[#7e3acb] text-white font-bold py-3 px-8 rounded-full shadow-sm transition-all flex items-center gap-2 w-full justify-center active:scale-95"
       >
        <Camera className="w-5 h-5"/>
