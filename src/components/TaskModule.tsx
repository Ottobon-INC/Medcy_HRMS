    ...
    <div className="space-y-2.5">
     <div className="flex items-center justify-between gap-2">
      <div className="flex items-center gap-1.5 flex-wrap">
       ...
       {isAssignedToMe && (
        <span className="px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider bg-[#f3edfb] text-[#7e3acb]">
     ...
     <div className="flex items-center gap-2">
      {assignedEmp ? (
       <div className="flex items-center gap-1.5"title={`Assigned to: ${assignedEmp.name}`}>
        <div className="w-5 h-5 rounded-full bg-[#f3edfb] text-[#7e3acb] flex items-center justify-center font-bold text-[9px]">
         {assignedEmp.name.split(' ').map(n => n[0]).join('').slice(0, 2)}
        </div>
        <span className={`text-[11px] font-medium truncate max-w-[120px] ${isAssignedToMe ? 'text-[#7e3acb] font-bold' : 'text-slate-600'}`}>
     ...
     <div className="flex items-center bg-slate-100 p-1 rounded-xl border border-slate-200/50">
      <button
       ...
       className={`p-1.5 rounded-lg transition-all cursor-pointer ${
        viewMode === 'list' ? 'bg-white text-[#7e3acb] shadow-sm' : 'text-slate-400 hover:text-slate-600'
       }`}
       title="List View"
      >
       ...
       >
        <div className="space-y-2 flex-1">
         <div className="flex items-center gap-2 flex-wrap">
          ...
          {getStatusBadge(task.status)}
          {isAssignedToMe && (
           <span className="px-2 py-0.5 rounded-md text-[10px] font-bold bg-[#f3edfb] text-[#7e3acb]">
       ...
       <div className="space-y-2">
        ...
        <div className="flex gap-2 pt-2">
         <input
          ...
          className="flex-1 px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-[#8a42db]/20 text-slate-700"
          onKeyDown={(e) => {
           if (e.key === 'Enter') {
            e.preventDefault();
         ...
         <button
          ...
          className="px-4 py-2.5 bg-[#8a42db] hover:bg-[#7e3acb] disabled:opacity-50 text-white font-bold rounded-xl text-xs uppercase tracking-wider transition-all cursor-pointer"
         >
