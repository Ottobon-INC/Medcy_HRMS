  ...
  <div id="dashboard-snapshot-container"className="space-y-6 sm:space-y-8 animate-fadeIn">
   ...
   <div className="bg-white rounded-[24px] sm:rounded-[32px] p-5 sm:p-6 border border-slate-100 shadow-sm flex flex-col md:flex-row items-center justify-between gap-5 sm:gap-6">
    <div className="flex items-center gap-4 md:pl-2 w-full md:w-auto">
     <div className={`w-12 h-12 rounded-full border flex items-center justify-center shrink-0 ${isOnBreak ? 'border-amber-200 bg-amber-50 text-amber-600' : isCheckedIn ? 'border-purple-200 bg-purple-50 text-purple-600' : 'border-slate-300 bg-white text-slate-600'}`}>
    ...
    <div className="flex items-center gap-3 w-full md:w-auto">
     ...
     <button onClick={hasCheckedOutToday ? undefined : isOnBreak ? handleResumeDuty : () => startCamera(false)} disabled={hasCheckedOutToday || isProcessing || isBreakLoading} className={`flex-1 md:flex-none px-8 py-3.5 rounded-full text-sm font-semibold transition-colors shadow-sm ${hasCheckedOutToday ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : isCheckedIn ? 'bg-rose-500 hover:bg-rose-600 text-white cursor-pointer' : 'bg-[#8a42db] hover:bg-[#7e3acb] text-white cursor-pointer'}`}>
   ...
   <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
    ...
    <div className="lg:col-span-6 space-y-6">
     ...
     <div className="bg-white rounded-[24px] sm:rounded-[32px] p-6 sm:p-8 shadow-sm border border-slate-100">
      <h3 className="text-base font-bold text-slate-800 mb-6 flex items-center justify-between">
       <span className="flex items-center gap-2"><Play className="text-[#8a42db] w-5 h-5" /> {language==="te"?"హాజరు సారాంశం":"Attendance Summary"}</span>
       <span className="text-[10px] font-bold text-[#8a42db] uppercase tracking-wider bg-[#f3edfb] px-2 py-1 rounded">This Month</span>
      </h3>
      <div className="flex justify-between items-end gap-2.5 h-24 my-4">
       ...
       <div className="flex-1 bg-[#8a42db] rounded-t-lg h-full shadow-md shadow-purple-500/20"/>
      </div>
      ...
      <button onClick={()=>setActiveTab("attendance")} className="mt-4 text-[13px] font-bold text-[#8a42db] hover:text-[#7e3acb] underline cursor-pointer">{language==="te"?"హాజరు షీట్ చూడండి":"View Full Sheet"}</button>
     </div>

     {/* Leave Portal */}
     <div className="bg-white rounded-[24px] sm:rounded-[32px] p-6 sm:p-8 shadow-sm border border-slate-100">
      <h3 className="text-base font-bold text-slate-800 mb-6 flex items-center gap-2">
       <Coffee className="text-[#8a42db] w-5 h-5" /> Leave Portal
      </h3>
      <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 flex justify-between items-center cursor-pointer hover:bg-slate-100 transition-colors group" onClick={()=>setActiveTab("leave")}>
       ...
       <div className="w-8 h-8 rounded-full bg-white shadow-sm flex items-center justify-center group-hover:translate-x-1 transition-all text-[#8a42db] font-bold">
        →
       </div>
      </div>
     </div>
    ...
    <div className="lg:col-span-6 space-y-6">
     ...
     <div className="bg-white rounded-[24px] sm:rounded-[32px] p-6 sm:p-8 shadow-sm border border-slate-100">
      <h3 className="text-base font-bold text-slate-800 mb-6 flex items-center justify-between">
       <span className="flex items-center gap-2"><MapPin className="text-[#8a42db] w-5 h-5" /> Field Operations</span>
     ...
     <div className="bg-white rounded-[24px] sm:rounded-[32px] p-6 sm:p-8 shadow-sm border border-slate-100">
      <h3 className="text-base font-bold text-slate-800 mb-6 flex items-center justify-between">
       <span className="flex items-center gap-2"><Camera className="text-[#8a42db] w-5 h-5" /> Field Visits</span>
       <span className="text-[10px] font-bold text-[#8a42db] uppercase tracking-wider bg-[#f3edfb] px-2 py-1 rounded">Photo Proof</span>
      </h3>
      <div className="py-2 mb-4">
      ...
      <button onClick={()=>setActiveTab("callCapture")} className="w-full py-3 bg-[#8a42db] hover:bg-[#7e3acb] text-white rounded-xl text-[13px] font-bold transition-all active:scale-95 cursor-pointer text-center shadow-sm">Start Field Visit Log</button>
     </div>
    </div>
    ...
    <div className="fixed inset-0 bg-slate-900/80 flex items-center justify-center p-4 z-50">
     <div className="bg-white rounded-2xl overflow-hidden max-w-md w-full shadow-md">
      <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
       <h3 className="font-bold text-slate-800 flex items-center gap-2"><Camera className="w-5 h-5 text-[#8a42db]"/>{language==="te"?"ఫోటో తీయండి":isCheckedIn?"Photo Punch-Out":"Photo Punch-In"}</h3>
      ...
      <div className="p-6 flex flex-col items-center gap-4">
       ...
       <button onClick={()=>{if(punchType==="out_of_office"&&!isCheckedIn&&!punchNote.trim()){alert(language==="te"?"దయచేసి ఒక నోట్ రాయండి.":"Please enter a location note.");return;}handleCaptureAndCheckIn();}} className="bg-[#8a42db] hover:bg-[#7e3acb] text-white font-bold py-3 px-8 rounded-full shadow-sm transition-all flex items-center gap-2 w-full justify-center active:scale-95"><Camera className="w-5 h-5"/>{language==="te"?(isCheckedIn?"ఫోటో తీసి పంచ్ అవుట్ చేయండి":"ఫోటో తీసి పంచ్ ఇన్ చేయండి"):(isCheckedIn?"Capture & Punch Out":"Capture & Punch In")}</button>
      </div>
     </div>
    </div>
   )}
   ...
   {isPinCameraOpen&&(
    <div className="fixed inset-0 bg-slate-900/80 flex items-center justify-center p-4 z-50">
     <div className="bg-white rounded-2xl overflow-hidden max-w-md w-full shadow-md">
      <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50"><h3 className="font-bold text-slate-800 flex items-center gap-2"><Camera className="w-5 h-5 text-[#8a42db]"/>{language==="te"?"పిన్ ఫోటో":"Pin Photo"}</h3><button onClick={stopCamera} className="p-1 hover:bg-slate-200 rounded-full transition-colors"><X className="w-5 h-5 text-slate-500"/></button></div>
      <div className="relative bg-black aspect-video"><video ref={videoRef} autoPlay playsInline muted className="w-full h-full object-cover"/><canvas ref={canvasRef} className="hidden"/><div className="absolute inset-0 border-4 border-[#8a42db]/30 m-4 rounded-xl pointer-events-none"/></div>
      <div className="p-6"><button onClick={handleCaptureAndPin} className="bg-[#8a42db] hover:bg-[#7e3acb] text-white font-bold py-3 px-8 rounded-full shadow-sm transition-all flex items-center gap-2 w-full justify-center active:scale-95"><Camera className="w-5 h-5"/>{language==="te"?"ఫోటో తీయండి":"Capture & Pin"}</button></div>
     </div>
    </div>
   )}
