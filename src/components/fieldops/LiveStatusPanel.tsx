    ...
    return (
      <div className="bg-white rounded-[24px] p-6 shadow-sm border border-slate-100">
        ...
        <p className="text-xs text-slate-500 font-medium leading-relaxed">
          No field agents are currently in transit (EN_ROUTE). When an agent starts a trip, live telemetry, distance, and ETA will appear here.
        </p>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-[24px] p-6 shadow-sm border border-slate-100 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className="px-2.5 py-1 text-[10px] font-black uppercase tracking-wider rounded-lg bg-amber-50 text-amber-700">
          {enRouteVisits.length} {enRouteVisits.length === 1 ? 'Agent Live' : 'Agents Live'}
        </span>
      </div>

      <div className="space-y-3">
        {enRouteVisits.map(visit => {
          const livePos = livePositions[visit.id];
          const route = routes[visit.id];
          const employeeName = getEmployeeName(visit.employeeId);
            ...
            >
              ...
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  <p className="text-xs text-slate-500 font-medium line-clamp-1">{visit.title}</p>
                </div>
